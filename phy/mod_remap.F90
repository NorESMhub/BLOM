...
contains
  ...
  subroutine remap(scp2i,scp2,pbmin,pbu,pbv,plo,cau,cav,mrg, &
                   dp,temp,saln,uflx,vflx,utflx,vtflx,usflx,vsflx,k)
    ...
    !   cau    - u-component of flux area.
    !   cav    - v-component of flux area.
    !   temp   - temperature.
    !   saln   - salinity.
    ...
    real,    intent(in) ,   dimension(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy) :: cau
    real,    intent(in) ,   dimension(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy) :: cav
    ...
    do j = 1-mrg-1,jj+mrg+1
      do l = 1,isu(j)
        do i = max(1-mrg,ifu(j,l)),min(ii+mrg+1,ilu(j,l))
          if (cau(i,j) > 0.) then
            cu(i,j) = cau(i,j)*scp2i(i-1,j)
          else
            cu(i,j) = cau(i,j)*scp2i(i  ,j)
          end if
        end do
      end do
    end do
    ...
    do j = 1-mrg,jj+mrg+1
      do l = 1,isv(j)
        do i = max(1-mrg-1,ifv(j,l)),min(ii+mrg+1,ilv(j,l))
          if (cav(i,j) > 0.) then
            cv(i,j) = cav(i,j)*scp2i(i,j-1)
          else
            cv(i,j) = cav(i,j)*scp2i(i,j  )
          end if
        end do
      end do
    end do
