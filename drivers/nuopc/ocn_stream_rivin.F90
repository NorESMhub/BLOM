module ocn_stream_rivin
  ...
  !*************************************************************************************************
   ...
   use ESMF              , only: ESMF_Mesh, ESMF_MeshCreate, ESMF_FieldCreate
   use ESMF              , only: ESMF_Field, ESMF_FieldCreate, ESMF_FieldGet
   use ESMF              , only: ESMF_FieldSMMStore, ESMF_FieldRegrid, ESMF_RouteHandle
...
!==============================================================================
   ...
   subroutine ocn_stream_rivin_init(mesh_model, omask, rc)
      ...
      real(r8), pointer      :: data_file(:)
      real(r8), pointer      :: data_model(:)
      integer                :: nf,i,j,n
      integer                :: jjcpl
      integer                :: index
      ...
      call xcbcst(stream_rivin_varname)
      ...
      if (mnproc == 1) then
         ...
         write(lp,'(a,a,a)') '  stream_rivin_varnames      = ',trim(stream_rivin_varname)
         write(lp,'(a)'   )  ' '
      endif
      ...
      do nf = 1,size(fldnames)
         ...
         ! Get pointer to data_file array
         call ESMF_FieldGet(field_file, farrayptr=data_file, rc=rc)
         if (chkerr(rc,__LINE__,sourcefile)) return
         ...
         call ESMF_FieldGet(field_model, farrayptr=data_model, rc=rc)
         if (chkerr(rc,__LINE__,sourcefile)) return
         ...
         do j = 1, jjcpl
            do i = 1, ii
               ...
               elseif (cplmsk(i,j) == 0) then
                  ...
                  n = (j - 1)*ii + i
                  rivflx_stream(i,j,index) = data_file(n)
               end if
