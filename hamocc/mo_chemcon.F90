...
module mo_chemcon
  ...
  !
  real(rp), parameter :: SALCHL=1._rp/1.80655_rp
  real(rp), parameter :: rrrcl=salchl*1.025_rp*bor1*bor2

  !     -----------------------------------------------------------------
  !*    ZERO DEG CENTIGRADE AT KELVIN SCALE
  !
  real(rp), parameter :: tzero=273.15_rp
  ...
  !
  real(rp), parameter :: CALCON=0.01028_rp

  !     -----------------------------------------------------------------
  !*    INVERS OF NORMAL MOLAL VOLUME OF AN IDEAL GAS [mol/ml] at 0C
  !
  real(rp), parameter :: OXYCO=1._rp/22414.4_rp
  ...
  !
  real(rp), parameter :: ac1= -162.8301_rp
  real(rp), parameter :: ac2=  218.2968_rp
  real(rp), parameter :: ac3=   90.9241_rp
  real(rp), parameter :: ac4=   -1.47696_rp
  real(rp), parameter :: bc1=    0.025695_rp
  real(rp), parameter :: bc2=   -0.025225_rp
  real(rp), parameter :: bc3=    0.0049867_rp
  ...
  !
  real(rp), parameter :: ad1= -60.2409_rp
  real(rp), parameter :: ad2=  93.4517_rp
  real(rp), parameter :: ad3=  23.3585_rp
  real(rp), parameter :: bd1=   0.023517_rp
  real(rp), parameter :: bd2=  -0.023656_rp
  real(rp), parameter :: bd3=   0.0047036_rp
  ...
  !     Marine Chemistry, 8, 347-359, 1980
  !
  real(rp), parameter :: al1= -165.8806_rp
  real(rp), parameter :: al2=  222.8743_rp
  real(rp), parameter :: al3=   92.0792_rp
  real(rp), parameter :: al4=   -1.48425_rp
  real(rp), parameter :: bl1=   -0.056235_rp
  real(rp), parameter :: bl2=    0.031619_rp
  real(rp), parameter :: bl3=   -0.0048472_rp

  !     -----------------------------------------------------------------
  !     NH3/NH4
  !     Tsilingiris 2008
  !     moist air dynamic viscosity parameters
  real(rp), parameter :: SV0_air =  1.715747771e-5_rp
  real(rp), parameter :: SV1_air =  4.722402075e-8_rp
  real(rp), parameter :: SV2_air = -3.663027156e-10_rp
  real(rp), parameter :: SV3_air =  1.873236686e-12_rp
  real(rp), parameter :: SV4_air = -8.050218737e-14_rp

  ! moist air density parameters
  real(rp), parameter :: SD0_air =  1.293393662_rp
  real(rp), parameter :: SD1_air = -5.538444326e-3_rp
  real(rp), parameter :: SD2_air =  3.860201577e-5_rp
  real(rp), parameter :: SD3_air = -5.2536065e-7_rp

  ! diffusion of NH3 in water and air
  real(rp), parameter :: Va_air  = 20.1_rp  ! Johnson 2010
  real(rp), parameter :: Ma_air  = 28.97_rp ! Johnson 2010
  real(rp), parameter :: Mb_nh3  = 17.03_rp ! Johnson 2010, Tang 2014
  real(rp), parameter :: Vb_nh3  = 20.7_rp  ! Johnson 2010
  real(rp), parameter :: M_nh3   =  (1._rp/Ma_air + 1._rp/Mb_nh3)**0.5_rp &
                                / (Va_air**(1._rp/3._rp)+Vb_nh3**(1._rp/3._rp))**2
  real(rp), parameter :: kappa   = 0.4_rp   ! von Karman constant

  !     -----------------------------------------------------------------
  !     Molecular weights
  real(rp), parameter :: mw_nitrogen = 14.00674_rp  ! [g/mol N]   nitrogen mol-weight as defined by CAM
  real(rp), parameter :: mw_nh3      = 17.028940_rp ! [g/mol NH3] ammonia mol-weight as defined by CAM
  real(rp), parameter :: mw_n2o      = 44.012880_rp ! [g/mol N2O] nitrous oxide mol-weight as defined by CAM
  real(rp), parameter :: mw_fe       = 55.85_rp     ! [g/mol Fe]  iron mol-weight
  ...
  real(rp), dimension(11) :: a0, a1, a2, b0, b1, b2
  ...
  real(rp), parameter :: rgas = 83.131_rp
