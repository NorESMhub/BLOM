...
module mo_param_bgc
  ...
  real(rp), protected :: perc_diron
  real(rp), protected :: fesoly     = 0.5_rp*1.e-9_rp    ! max. diss. iron concentration in deep water
  ...
  !********************************************************************
  real(rp), protected :: bkopal     = 5.e-6_rp        ! kmol/m3 - i.e. 4.0 mmol Si/m3 half saturation constant
  ...
  real(rp), protected :: bkphyanh4     = 0.25e-6_rp  ! Half-saturation constant for NH4 uptake by bulk phytoplankton (kmol/m3)
  ...
  real(rp), protected :: ranh4nitr     = 1.0_rp      ! Maximum growth rate nitrification on NH4 at reference T (1/d -> 1/dt)
  ...
  real(rp), protected :: rano3denit    = 0.00005_rp  ! Maximum growth rate denitrification on NO3 at reference T (1/d -> 1/dt)
  ...
  real(rp), protected :: wmin        =  1._rp           ! m/d   minimum sinking speed
  ...
  real(rp), protected :: disso_poc   = 0.01_rp/sec_per_day ! 1/(kmol O2/m3 s) disso=3.e-5 was quite high - Degradation rate constant of POP
  real(rp), protected :: disso_sil   = 3.e-8_rp            ! 1/(kmol Si(OH)4/m3 s) Dissolution rate constant of opal
...
contains
  ...
  subroutine ini_param_biol()
    ...
    if (use_AGG) then
      ...
      epsher  = 0.85_rp       ! dimensionless fraction -fraction of grazing egested
    else
    ...
    if (use_AGG) then
      ...
      rcalc  = 33._rp         ! calcium carbonate to organic phosphorous production ratio
      ropal  = 45._rp         ! opal to organic phosphorous production ratio
    else
