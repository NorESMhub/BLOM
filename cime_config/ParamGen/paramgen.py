...
class ParamGen:
    ...
    def from_xml_nml(cls, input_path, match="last", no_duplicates=False):
        ...
        for entry in list(root):
            ...
            if "is_inputdata" in entry.attrib:
                data[param_name]["is_inputdata"] = entry.attrib["is_inputdata"]
            else:
                data[param_name]["is_inputdata"] = "no"

            if "modify_via_xml" in entry.attrib:
                data[param_name]["modify_via_xml"] = entry.attrib["modify_via_xml"]
            else:
                data[param_name]["modify_via_xml"] = 'unset'
