...
! ------------------------------------------------------------------------------
   ...
   use mod_constants, only: g, alpha0
...
contains
   ...
   subroutine cntiso_hybrid_vdiff(m, n, mm, nn, k1m, k1n)
      ...
      do j = 1, jj
         ...
         do i = max(1, ifp(j, l)), min(ii, ilp(j, l))
            ...
            temp_1d(1) = dp_1d(1)*temp_1d(1)*bei
            do k = 2, kk - 1
               gam(k) = - fp(k)*bei
               bei = 1._r8/(dp_1d(k) + fp(k)*(1._r8 + gam(k)) + fp(k + 1))
               temp_1d(k) = (dp_1d(k)*temp_1d(k) + fp(k)*temp_1d(k - 1))*bei
            enddo
            gam(kk) = - fp(kk)*bei
            bei = 1._r8/(dp_1d(kk) + fp(kk)*(1._r8 + gam(kk)))
            temp_1d(kk) = (dp_1d(kk)*temp_1d(kk) + fp(kk)*temp_1d(kk - 1))*bei
            do k = kk - 1, 1, - 1
            ...
            enddo
            bei = 1._r8/(dp_1d(1) + fp(2))
            saln_1d(1) = dp_1d(1)*saln_1d(1)*bei
            do k = 2, kk - 1
               gam(k) = - fp(k)*bei
               bei = 1._r8/(dp_1d(k) + fp(k)*(1._r8 + gam(k)) + fp(k + 1))
               saln_1d(k) = (dp_1d(k)*saln_1d(k) + fp(k)*saln_1d(k - 1))*bei
            enddo
            gam(kk) = - fp(kk)*bei
            bei = 1._r8/(dp_1d(kk) + fp(kk)*(1._r8 + gam(kk)))
            saln_1d(kk) = (dp_1d(kk)*saln_1d(kk) + fp(kk)*saln_1d(kk - 1))*bei
            do k = kk - 1, 1, - 1
            ...
            bei = 1._r8/(dp_1d(1) + fp(2))
            do nt = 1, ntr
               trc_1d(1, nt) = dp_1d(1)*trc_1d(1, nt)*bei
            enddo
            do k = 2, kk - 1
               gam(k) = - fp(k)*bei
               bei = 1._r8/(dp_1d(k) + fp(k)*(1._r8 + gam(k)) + fp(k + 1))
               do nt = 1, ntr
                  trc_1d(k, nt) = ( dp_1d(k)*trc_1d(k, nt) &
                                  + fp(k)*trc_1d(k - 1, nt))*bei
               enddo
            enddo
            gam(kk) = - fp(kk)*bei
            bei = 1._r8/(dp_1d(kk) + fp(kk)*(1._r8 + gam(kk)))
            do nt = 1, ntr
               trc_1d(kk, nt) = ( dp_1d(kk)*trc_1d(kk, nt) &
                                + fp(kk)*trc_1d(kk - 1, nt))*bei
            enddo
            do k = kk - 1, 1, - 1
               do nt = 1, ntr
   ...
   end subroutine cntiso_hybrid_vdiff
