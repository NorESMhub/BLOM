  ...
  subroutine difest_common_iso(m,n,mm,nn,k1m,k1n)
    ...
    do j = 1,jj+1
      ...
      do k = kk,4,-1
        ...
        do l = 1,isv(j)
          do i = max(1,ifv(j,l)),min(ii,ilv(j,l))
            if (k >= max(kfil(i,j-1),kfil(i,j)).and. &
                 dpv(i,j,kn) > dptmin) kfpl(i) = k
          end do
        end do
      end do
      do k = 1,kk
        ...
        do l = 1,isv(j)
          do i = max(1,ifv(j,l)),min(ii,ilv(j,l))
            if (k >= kfpl(i).and.k <= klpl(i).and. &
                 klpl(i)-kfpl(i) >= 1) then
              if     (k == kfpl(i)) then
    ...
    do j = 1,jj
      ...
      do k = kk,4,-1
        ...
        do l = 1,isu(j)
          do i = max(1,ifu(j,l)),min(ii+1,ilu(j,l))
            if (k >= min(kfil(i-1,j),kfil(i,j)).and. &
                 dpu(i,j,kn) > dptmin) kfpl(i) = k
          end do
        end do
      end do
      do k = 1,kk
        ...
        do l = 1,isp(j)
          do i = max(1,ifp(j,l)),min(ii,ilp(j,l))
            if (msku(i,j,k)+msku(i+1,j,k) &
                 +mskv(i,j,k)+mskv(i,j+1,k) > 0) then
